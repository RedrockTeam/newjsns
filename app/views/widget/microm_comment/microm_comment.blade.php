...
<div class="m-microm_comments">
    @for($i = 0; $i < count($movie_comments); $i++)
        <div class="u-comment_item f-cb">
            <img class="u-show_user_icon" src="{{$movie_comments[$i]['user_icon']}}" alt=""/>
            <aside class="u-comment_detail">
                <span class="u-user_name">{{$movie_comments[$i]['user_name']}}</span>
                <p class="u-user_content"> {{$movie_comments[$i]['user_comment']}}</p>
                @for($j = 0; $j < count($movie_comments[$i]['user_replys']); $j++)
                <div class="s-reply_items">
                    <span class="u-user_name">{{$movie_comments[$i]['user_replys'][$j]['user_name']}}</span>
                    <span class="u-reply_tag">回复</span>
                    <span class="u-user_name">{{$movie_comments[$i]['user_replys'][$j]['reply_to_name']}}</span>
                    <span class="u-reply_content">{{$movie_comments[$i]['user_replys'][$j]['reply_content']}}</span>
                </div>
                @endfor
                <div class="u-user_action">
                    <span>赞</span>
                    <span>回复</span>
                    <span>{{$movie_comments[$i]['comment_date']}}</span>
                </div>
            </aside>
        </div>
    @endfor
