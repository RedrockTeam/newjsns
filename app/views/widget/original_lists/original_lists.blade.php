    ...
    <dl class="m-ori_print_lists g-lists f-cb">
        ...
        @foreach($data['main'][$key] as $work)
            <dd class="u-list_item">
                <a href="passage_id={{$work['id']}}&type_id={{$work['type_id']}}"  class="@if($work['type_id'] == 16 || $work['type_id'] == 17){{"js-open_model"}}@endif" data-type_id="{{$work['type_id']}}" data-passage_id="{{$work['id']}}" data-love_num="{{$work['love_num']}}" data-intro="{{$work['introduce']}}" data-author="{{$work['author']}}">
